    ...
    public static void filterSpam(MessageReceivedEvent event) { 
        ...
        try {
            if (!event.getChannel().getId().equals("996261782693875835")) {
                List<Message> history = event.getChannel().asTextChannel().getIterableHistory().complete().stream().limit(14).filter(msg -> !msg.equals(event.getMessage())).collect(Collectors.toList());
