...
public class StarBoard {
    public static void system(MessageReactionAddEvent event) {
        ...
        //if (event.getGuild().getId().equals("1010933101939130462")) { //lock for testing ⭐ DateTimeFormatter.ofPattern("MM/dd/yyyy"); 
    ...
    private static void newStarBoard(MessageReactionAddEvent event) {
        ...
        .sendMessageEmbeds(embed.build()).addContent(replyStar(stars) + " " + stars + " " + channelMention).queue(m -> {
    ...
    private static boolean starableMessage(MessageReactionAddEvent event) {
        ...
        !event.retrieveMessage().complete().getChannel().asTextChannel().isNSFW() && time >= 20220629) {
            return true;
