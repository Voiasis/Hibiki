...
public class prefixUserinfo {
    ...
    private static void send(Message message, User user) {
        ...
        embed.addField("User ID", "<:idblock:994818038983561236> " + user.getId(), false);
        embed.addField("Joined Discord", "<:discord:995010267400376391> " + "<t:" + createTime + ":d> <t:" + createTime + ":t> (" + jdDiff + " " + Grammar.grammar("day", jdDiff) + " ago)", false);
        if (user.getAvatarUrl() != null) {
            ...
            if (message.isFromGuild()) {
                ...
                embed.addField("Joined Server", "<:time:994820769282531450> " + "<t:" + joinTime + ":d> <t:" + joinTime + ":t> (" + jsDiff + " " + Grammar.grammar("day", jsDiff) + " ago)", false);
                if (user.retrieveProfile().complete().getBanner() != null) {
                    embed.setImage(user.retrieveProfile().complete().getBannerUrl() + "?size=512");
                    embed.setDescription("<:mention:995754056519778477> " + user.getAsMention());
