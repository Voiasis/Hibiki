...
public class slashActinfo extends CommandBuilder {
    public slashActinfo() {
        setCommandData(Commands.slash("actinfo", "Lists users current activities.").addOption(OptionType.USER, "user", "Who to check activities from.", false));
    }
    @Override
    ...
    private static void send(SlashCommandInteractionEvent event, Member member) {
        ...
        if (!member.getActivities().isEmpty()) {
            ...
            switch (actCount) {
                ...
                event.reply(Grammar.grammarY("Activity", actCount) +" for " + member.getEffectiveName()).setEmbeds(embed1.build()).mentionRepliedUser(false).queue();
                break;
                case 2 :
                ...
                event.reply(Grammar.grammarY("Activity", actCount) +" for " + member.getEffectiveName()).setEmbeds(embed1.build(), embed2.build()).mentionRepliedUser(false).queue();
                break;
                case 3 :
                ...
                event.reply(Grammar.grammarY("Activity", actCount) +" for " + member.getEffectiveName()).setEmbeds(embed1.build(), embed2.build(), embed3.build()).mentionRepliedUser(false).queue();
                break;
                case 4 :
                ...
                event.reply(Grammar.grammarY("Activity", actCount) +" for " + member.getEffectiveName()).setEmbeds(embed1.build(), embed2.build(), embed3.build(), embed4.build()).mentionRepliedUser(false).queue();
                break;
                case 5 :
                ...
                event.reply(Grammar.grammarY("Activity", actCount) +" for " + member.getEffectiveName()).setEmbeds(embed1.build(), embed2.build(), embed3.build(), embed4.build(), embed5.build()).mentionRepliedUser(false).queue();
                break;
                case 6 :
                ...
                event.reply(Grammar.grammarY("Activity", actCount) +" for " + member.getEffectiveName()).setEmbeds(embed1.build(), embed2.build(), embed3.build(), embed4.build(), embed5.build(), embed6.build()).mentionRepliedUser(false).queue();
                break;
            }
            if (actCount >= 7) {
                if (event.getMember() == member) {
                    event.reply("Woah woah woah.. You need to slow down. You are multitasking WAYYY too hard.").mentionRepliedUser(false).queue();
                } else {
                    event.reply("Woah woah woah.. " + member.getEffectiveName() + " needs to slow down. They are multitasking WAYYY too hard.").mentionRepliedUser(false).queue();
                }
            }
        } else {
            if (event.getMember() == member) {
                event.reply("You do not currently have any activities.").mentionRepliedUser(false).queue();
            } else {
                event.reply(member.getEffectiveName() + " does not currently have any activities.").mentionRepliedUser(false).queue();
            }
        }
    }
