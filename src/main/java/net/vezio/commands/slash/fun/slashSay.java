package net.vezio.commands.slash.fun;
        ...
        if (allowedUsers(member)) {
            ...
            if (channel == null) {
                if (replyId == null) {
                    ...
                    TextChannel channel2 = event.getChannel().asTextChannel();
            ...
            event.reply("No access.").mentionRepliedUser(false).setEphemeral(true).queue();
        }
    }
    public static boolean allowedUsers(Member member) {
        String userId = member.getId();
        if (userId.equals("835691330725347379")) { //me
            return true;
